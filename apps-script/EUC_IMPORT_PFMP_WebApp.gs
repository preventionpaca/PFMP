/** Eucalyptus PFMP — v1.0.0-dev.69 — route import Pronote + diagnostic précis des blocages LP/LGT. */
function EUC_IMPORT_afficherApplication(){
  ...
  tpl.config=JSON.stringify({version:'Eucalyptus PFMP — v1.0.0-dev.69',mode:'IMPORT_REEL_CONTROLE',role:ctx.role,baseUrl:ScriptApp.getService().getUrl()});
