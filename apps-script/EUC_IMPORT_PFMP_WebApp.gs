/** Eucalyptus PFMP — v1.0.0-dev.68 — route import Pronote + liste complète analysée. */
function EUC_IMPORT_afficherApplication(){
  ...
  tpl.config=JSON.stringify({version:'Eucalyptus PFMP — v1.0.0-dev.68',mode:'IMPORT_REEL_CONTROLE',role:ctx.role,baseUrl:ScriptApp.getService().getUrl()});
