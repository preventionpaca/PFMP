/** Eucalyptus PFMP — v1.0.0-dev.66 — route import Pronote protégée + feedback visuel + correction dates Grist. */
function EUC_IMPORT_afficherApplication(){
  ...
  tpl.config=JSON.stringify({version:'Eucalyptus PFMP — v1.0.0-dev.66',mode:'IMPORT_REEL_CONTROLE',role:ctx.role,baseUrl:ScriptApp.getService().getUrl()});
