/** Eucalyptus PFMP — v1.0.0-dev.82 — import Pronote Professeurs + Classes, liaisons PP et diplômes par classe. */
...
function EUC_PC_ensure_(){
  var tables=EUC_ENT_grist('get','/tables').tables||[], ids={}; tables.forEach(function(t){ids[t.id]=true;});
  var c=EUC_PC_col_;
  if(!ids[EUC_PROFS_TABLE_])EUC_ENT_grist('post','/tables',{tables:[{id:EUC_PROFS_TABLE_,columns:[
    c('Identifiant_Pronote','Identifiant Pronote'),c('Numero_Pronote','Numéro Pronote'),c('Civilite','Civilité'),c('Nom','Nom'),c('Prenom','Prénom'),c('Date_naissance','Date de naissance','Date'),c('Discipline','Discipline'),c('Code_matiere_pref','Code matière préférentielle'),c('Matiere_pref','Matière préférentielle'),c('Email','E-mail'),c('Telephone_fixe','Téléphone fixe'),c('Telephone_portable','Téléphone portable'),c('Classes_source','Classes source'),c('Actif','Actif','Bool'),c('Date_modification','Date modification','DateTime')
  ]}]});
  if(!ids[EUC_CLASSES_PROFS_TABLE_])EUC_ENT_grist('post','/tables',{tables:[{id:EUC_CLASSES_PROFS_TABLE_,columns:[
    c('Classe','Classe','Ref:Classes'),c('Professeur','Professeur','Ref:'+EUC_PROFS_TABLE_),c('Role','Rôle'),c('Source_Pronote','Source Pronote'),c('Actif','Actif','Bool'),c('Date_modification','Date modification','DateTime')
  ]}]});
  if(!ids[EUC_CLASSES_DIPLOMES_TABLE_])EUC_ENT_grist('post','/tables',{tables:[{id:EUC_CLASSES_DIPLOMES_TABLE_,columns:[
    c('Classe','Classe','Ref:Classes'),c('Intitule_diplome','Intitulé exact du diplôme'),c('Actif','Actif','Bool'),c('Date_modification','Date modification','DateTime'),c('Auteur','Auteur')
  ]}]});
  var existing=EUC_ENT_grist('get','/tables/Classes/columns').columns||[], present={}; existing.forEach(function(x){present[x.id]=true;});
  var wanted=[c('Etablissement_Pronote','Établissement Pronote'),c('Niveau_Pronote','Niveau Pronote'),c('Filiere_Pronote','Filière Pronote'),c('Principaux_Pronote','Professeurs principaux Pronote')];
  var missing=wanted.filter(function(x){return !present[x.id];}); if(missing.length)EUC_ENT_grist('post','/tables/Classes/columns',{columns:missing});
}
...
function EUC_PC_importerClasses(payload){
  ...
  var ix={nom:EUC_PC_ix_(h,'NOM'),niveau:EUC_PC_ix_(h,'NIVEAU'),etab:EUC_PC_ix_(h,'ETABLISSEMENT'),principaux:EUC_PC_ix_(h,'PRINCIPAUX'),filiere:EUC_PC_ix_(h,'FILIERE')};
  var classes=EUC_IMPORT_lireRecordsBruts_('Classes'), byNom={}; classes.forEach(function(r){var f=r.fields||r; [f.Nom,f.Libelle,f.Code_import].forEach(function(x){if(x)byNom[EUC_PC_norm_(x)]=r;});});
  ...
  var classPatches=[],linkPosts=[],linkPatches=[],unknown=[],unknownProfs=[],now=EUC_IMPORT_nowGrist_();
  p.rows.forEach(function(r){var name=String(r[ix.nom]||'').trim(),cl=byNom[EUC_PC_norm_(name)]; if(!cl){unknown.push(name);return;} var cf={Etablissement_Pronote:String(r[ix.etab]||''),Niveau_Pronote:String(r[ix.niveau]||''),Filiere_Pronote:String(r[ix.filiere]||''),Principaux_Pronote:String(r[ix.principaux]||'')}; classPatches.push({id:cl.id,fields:cf});
    String(r[ix.principaux]||'').split(',').map(function(x){return x.trim();}).filter(Boolean).forEach(function(label){var prof=byName[EUC_PC_norm_(label)]; if(!prof){unknownProfs.push(name+' → '+label);return;} var k=[cl.id,prof.id,'PROFESSEUR_PRINCIPAL'].join('|'),fields={Classe:cl.id,Professeur:prof.id,Role:'PROFESSEUR_PRINCIPAL',Source_Pronote:String(r[ix.etab]||''),Actif:true,Date_modification:now}; if(byLink[k])linkPatches.push({id:byLink[k].id,fields:fields}); else linkPosts.push({fields:fields});});
  });
