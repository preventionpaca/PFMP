...
function EUC_IMPORT_normaliserEntete_(v){return String(v||'').replace(/^\uFEFF/,'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toUpperCase().replace(/[’'_\-]+/g,' ').replace(/[()]/g,'').replace(/\s+/g,' ').trim();}
function EUC_IMPORT_lireCSV_(line,sep){var out=[],v='',q=false;for(var i=0;i<line.length;i++){var c=line[i];if(c==='"'){if(q&&line[i+1]==='"'){v+='"';i++;}else q=!q;}else if(c===sep&&!q){out.push(v);v='';}else v+=c;}if(q)throw new Error('Guillemets non fermés.');out.push(v);return out;}
function EUC_IMPORT_decoderBase64_(base64){var bytes=Utilities.base64Decode(String(base64||''));if(!bytes.length)throw new Error('Fichier vide.');if((bytes[0]&255)===255&&(bytes[1]&255)===254){var chars=[];for(var i=2;i+1<bytes.length;i+=2)chars.push(String.fromCharCode((bytes[i]&255)|((bytes[i+1]&255)<<8)));return {encoding:'UTF-16LE',text:chars.join('')};}if((bytes[0]&255)===254&&(bytes[1]&255)===255)throw new Error('UTF-16BE non pris en charge. Exportez en UTF-16LE ou UTF-8.');return {encoding:'UTF-8',text:Utilities.newBlob(bytes).getDataAsString('UTF-8').replace(/^\uFEFF/,'')};}
function EUC_IMPORT_detecterSeparateur_(header){var choices=['\t',';',','],scores=choices.map(function(s){return {s:s,n:EUC_IMPORT_lireCSV_(header,s).length};}).sort(function(a,b){return b.n-a.n;});if(scores[0].n<3||scores[0].n===scores[1].n)throw new Error('Séparateur ou type d’export non reconnu.');return scores[0].s;}
function EUC_IMPORT_empreinte_(bytes){return EUC_SUIVI_octetsHex_(Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256,bytes));}
function EUC_IMPORT_indexAlias_(headers,aliases){for(var i=0;i<aliases.length;i++){var x=headers.indexOf(EUC_IMPORT_normaliserEntete_(aliases[i]));if(x>=0)return x;}return -1;}
function EUC_IMPORT_valeur_(cells,index){return index>=0?String(cells[index]||'').trim():'';}
function EUC_IMPORT_analyserTexte_(text,options){
  options=options||{};
  var annee=String(options.annee||'').trim(),annees=annee.match(/^(20\d{2})-(20\d{2})$/);
  if(!annees||Number(annees[2])!==Number(annees[1])+1)throw new Error('Année scolaire obligatoire ou incohérente.');
...
function EUC_IMPORT_previsualiserBase64(payload){var ctx=EUC_SUIVI_contexteCourant_();if(!ctx.autorise||['DDFPT','ADMIN_PFMP','BUREAU_ENTREPRISES'].indexOf(ctx.role)<0)throw new Error('Accès non autorisé.');payload=payload||{};var decoded=EUC_IMPORT_decoderBase64_(payload.base64),parsed=EUC_IMPORT_analyserTexte_(decoded.text,{annee:payload.annee});parsed.encoding=decoded.encoding;parsed.empreinte=EUC_IMPORT_empreinte_(Utilities.base64Decode(String(payload.base64||'')));var existing=EUC_SUIVI_fields_(EUC_SUIVI_sqlLecture_("SELECT e.*,a.Code AS Annee_code FROM EUC_ELEVES_PFMP e JOIN Annees_Scolaires a ON a.id=e.Annee_scolaire WHERE a.Code=?",[parsed.annee])),offers=EUC_SUIVI_fields_(EUC_ENT_grist('get','/tables/EUC_OFFRES_FORMATION/records')),imports=[];try{imports=EUC_SUIVI_fields_(EUC_ENT_grist('get','/tables/EUC_IMPORTS_PRONOTE_PFMP/records'));}catch(e){}return EUC_IMPORT_previsualiser_(parsed,existing,offers,imports);}
function EUC_IMPORT_confirmerSimulation(payload){var lock=LockService.getScriptLock();if(!lock.tryLock(1000))throw new Error('Un autre import est déjà en cours.');try{if(!payload||payload.confirmation!=='CONFIRMER_IMPORT')throw new Error('Validation humaine explicite obligatoire.');var mode=PropertiesService.getScriptProperties().getProperty('EUC_PFMP_PRONOTE_IMPORT_MODE')||'DRY_RUN';if(mode!=='DRY_RUN')throw new Error('Import réel non autorisé dans cette version.');var preview=EUC_IMPORT_previsualiserBase64(payload);if(!preview.pretAValider)throw new Error(preview.dejaImporte?'Fichier identique déjà importé.':'Prévisualisation bloquée par des anomalies.');return {statut:'SIMULATION',mode:'DRY_RUN',ecriture:false,empreinte:preview.empreinte,compteurs:preview.compteurs};}finally{lock.releaseLock();}}
