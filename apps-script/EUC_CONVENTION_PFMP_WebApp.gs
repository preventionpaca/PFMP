/** Eucalyptus PFMP — v1.0.0-dev.94 — générateur Word natif, QR court et lot unique imprimable. */
function EUC_CONVENTION_afficherGenerateur(){
  ...
  var tpl=HtmlService.createTemplateFromFile('Convention_PFMP_Generateur');tpl.config=JSON.stringify({version:'Eucalyptus PFMP — v1.0.0-dev.94',baseUrl:ScriptApp.getService().getUrl()});
...
function EUC_CONVENTION_afficherImpressionLot(e){
  ...
  var d=EUC_CONVENTION_donneesImpressionLot(lot),tokens=(d.items||[]).map(function(i){return i.token;}).filter(Boolean),r=EUC_DOCX_genererLotImprimableV94(tokens);
  return EUC_CONVENTION_htmlLienDrive_('Lot PFMP Word',r.total+' conventions regroupées dans un seul document Word imprimable.',r.url,r.folderUrl);
}
