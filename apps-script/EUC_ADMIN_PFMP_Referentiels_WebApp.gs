/** Eucalyptus PFMP — v1.0.0-dev.91 — pages administration référentiels Pronote, diplômes et paramètres convention. */
function EUC_PC_afficherSynchronisation(){
  ...
  tpl.config=JSON.stringify({version:'Eucalyptus PFMP — v1.0.0-dev.91',role:ctx.role,baseUrl:ScriptApp.getService().getUrl()});
...
function EUC_PARAM_CONV_afficher(){
  ...
  // IMPORTANT : l'HTML attend un objet JavaScript sérialisé dans `const C=...`.
  // Un objet Apps Script brut est rendu sous une forme non-JavaScript et bloque tout le script client.
  tpl.config=JSON.stringify({version:'Eucalyptus PFMP — v1.0.0-dev.91',role:ctx.role,baseUrl:ScriptApp.getService().getUrl()});
