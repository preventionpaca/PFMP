...
function EUC_IMPORT_lireRecordsBruts_(table){
  try{return (EUC_ENT_grist('get','/tables/'+encodeURIComponent(table)+'/records').records||[]);}catch(e){return [];}
}
function EUC_IMPORT_lireRecords_(table){
  return EUC_IMPORT_lireRecordsBruts_(table).map(function(r){return Object.assign({id:r.id},r.fields||{});});
}
function EUC_IMPORT_chargerClassesCamin_(){
  return EUC_IMPORT_lireRecordsBruts_('Classes').map(function(r){
    var f=r.fields||{};
    return {id:r.id,nom:String(f.Nom||f.Libelle||'').trim(),libelle:String(f.Libelle||f.Nom||'').trim(),formation:String(f.Formation||'').trim(),niveau:String(f.Niveau||'').trim(),actif:f.Actif!==false};
  }).filter(function(r){return r.id&&r.nom;});
}
...
function EUC_IMPORT_chargerDonneesPreviewTexte_(annee){
  ...
  try{imports=EUC_IMPORT_lireRecords_('EUC_IMPORTS_PRONOTE_PFMP');}catch(e){}
