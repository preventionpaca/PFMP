/** Eucalyptus PFMP — v1.0.0-dev.97 — vérification publique QR court robuste. */
function EUC_CONVENTION_trouverAccesParCodeV94_(code){
  ...
  var rows=EUC_IMPORT_lireRecords_('EUC_ACCES_FORMULAIRES_PFMP'),matches=rows.filter(function(r){
    if(r.Revoked===true)return false;
    var h=String(r.Token_hash||'');
    return h&&h.substring(0,code.length)===code;
  });
