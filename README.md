...
La version `dev.25` sépare le référentiel multiannuel des cohortes et charge séparément les compteurs depuis une synthèse non nominative. La liste nominative reste filtrée côté serveur après sélection. La seule cible autorisée reste la copie `j1jDArBkzi7P` ; les gardes `DRY_RUN` et `DISABLED` restent actives. La version Apps Script immuable `24` est publiée sur le déploiement de recette existant.
